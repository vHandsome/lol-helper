...
public class LeagueClientService {
	...
	public ArrayList<String> dealScore2Msg(List<String> puuidList) throws IOException {
		...
		for (String id : puuidList) {
			//查询战绩
			List<ScoreBO> scoreBOList = api.getScoreById(id, 5);
			// 计算得分 最近三把(KDA+输赢)的平均值
			// KDA->(击杀*1.2+助攻*0.8)/(死亡*1.2)
			// 输赢->赢+1 输-1
			int score = 0;
			Integer kills = 0, assists = 0, deaths = 0;
			for (ScoreBO scoreBO : scoreBOList) {
				...
				kills += scoreBO.getKills();
				deaths += scoreBO.getDeaths();
				assists += scoreBO.getAssists();
			}
			//查看玩家名称
			...
			result.add("<"+displayName+">"+ "最近5局表现 击杀：" + kills + "，助攻：" + assists + "，死亡：" + deaths);
		}
		Map.Entry<Integer, String> firstEntry = treeMap.firstEntry();
		Map.Entry<Integer, String> lastEntry = treeMap.lastEntry();
		result.add("");
		result.add("牛马是:<" + firstEntry.getValue() + "> 5局胜场:" + firstEntry.getKey().toString());
		result.add("大神是:<" + lastEntry.getValue() + "> 5局胜场:" + lastEntry.getKey().toString());
		return result;
	}
